...
Long memberId = jwtUtil.getId(accessToken);
    ...
    public ResponseEntity<Response<CommentResponse>> updateBoardComment(@PathVariable long id, @RequestBody CommentRequest commentRequest, @RequestHeader("Authorization") String accessToken) {
        Long memberId = jwtUtil.getId(accessToken);
        return ResponseEntity.ok(Response.ok(commentService.updateBoardComment(commentRequest, id, memberId)));
    }

    @GetMapping("/boards/{boardId}")
    public ResponseEntity<Response<List<CommentResponse>>> getBoardComment(@PathVariable long boardId, @RequestHeader("Authorization") String accessToken) {
        Long memberId = jwtUtil.getId(accessToken);
        return ResponseEntity.ok(Response.ok(commentService.getBoardComments(boardId)));
    }

    @DeleteMapping("/{id}/boards")
    public ResponseEntity<Response<String>> deleteBoardComment(@PathVariable long id, @RequestHeader("Authorization") String accessToken) {
        Long memberId = jwtUtil.getId(accessToken);
